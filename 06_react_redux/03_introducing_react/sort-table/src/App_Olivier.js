...
let productFinal = products;

class App extends Component {

  sortById(id) {
    console.log("_.sortBy(",products, id,")");
    productFinal = _.sortBy(products, id);
  ...
  render() {
    return (
      <div className="App" class="thTd">
        <table class="table">
          <tr>
		        <th onClick={()=> this.sortById("decathlon_id")}>Id</th>
            <th onClick={()=> console.log("Title");}>Title</th>
            <th onClick={()=> console.log("Price")}>Price</th>
          </tr>
            {productFinal.map(element => <tr><td>{element.decathlon_id}</td><td>{element.title}</td><td>{element.price}</td></tr>)}
        </table>
      </div>
    );
  }
